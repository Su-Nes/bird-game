...
const NATIVE_OCCLUSION_TRI_LIMIT := 8000000
# Baked falloff resolution handed to the native brush
const FALLOFF_SAMPLES := 256
...
#region Native acceleration
# ----------------------------------------------------------------
# Native acceleration (optional compiled core)
# ----------------------------------------------------------------

func all_native() -> bool:
	if targets.is_empty():
		return false
	for md in targets:
		if md.nat == null:
...
func sync_native_view(camera: Camera3D) -> void:
	if camera == null:
		return
	...
	for md in targets:
		if md.nat == null or not is_instance_valid(md.mesh_instance):
			continue
		md.nat.set_transform(md.mesh_instance.global_transform)
		md.nat.set_view(camera.global_transform, camera.get_camera_projection(),
			camera.projection, camera.near, vp_size)


func push_native_brush() -> void:
	var mask := 0
	var cm := state.channel_mask()
	for i in 4:
		if cm[i]:
			mask |= 1 << i
	var falloff := PackedFloat32Array()
	if state.falloff != null:
		falloff.resize(FALLOFF_SAMPLES)
		for i in FALLOFF_SAMPLES:
			falloff[i] = clampf(state.falloff.sample_baked(float(i) / float(FALLOFF_SAMPLES - 1)), 0.0, 1.0)
	var cfg := {
		"op": state.operation,
		"color": state.color,
		...
		"radius": state.radius,
		"mask": mask,
		"single": state.is_single_channel(),
		"value": state.channel_value,
		"falloff": falloff,
		"merge": state.shared_verts == VSState.SharedVerts.MERGE,
		"split": state.shared_verts == VSState.SharedVerts.SPLIT,
		"front_only": state.front_verts_only,
		"do_occ": total_triangles() <= NATIVE_OCCLUSION_TRI_LIMIT,
		"ui_scale": _ui_scale(),
	...
	for md in targets:
		if md.nat != null:
			md.nat.set_brush(cfg)
			md.nat.set_use_selection(sel_on)
...
func begin_stroke() -> void:
	...
	for md in targets:
		if md.nat != null:
			md.nat.begin_stroke()
			_stroke_origin.append(null)
			_stroke_weight.append(null)
			continue
...
func end_stroke_collect() -> Array:
	...
	for md in targets:
		if md.nat == null:
			out.append(null)
			continue
		var d: Dictionary = md.nat.end_stroke()
		var idx = d.get("indices")
		out.append(d if idx is PackedInt32Array and not (idx as PackedInt32Array).is_empty() else null)
	return out
...
func select_marquee(camera: Camera3D, pts: PackedVector2Array, type: int, mode: int) -> void:
	...
	for md in targets:
		if md.nat != null:
			md.nat.select_marquee(pts, type, 1 if mode == 0 else mode)
			continue
...
func select_point(camera: Camera3D, mouse: Vector2, mode: int) -> bool:
	var best := maxf(state.dot_size, 8.0) * _ui_scale()
	...
	for md in targets:
		if not is_instance_valid(md.mesh_instance):
			continue
		if md.nat != null:
			var nfi: int = md.nat.pick_vertex(mouse, best, true)
			if nfi >= 0:
				var sp2: Vector2 = md.nat.screen_pos(nfi) + md.nat.fan_offset(nfi)
				var nd := sp2.distance_to(mouse)
				if nd < best:
					best = nd
					bmd = md
					bfi = nfi
...
func sync_for_save() -> void:
	for md in targets:
		if md.needs_save_sync():
			md.rebuild()
...
func sample(camera: Camera3D, mouse: Vector2, max_px := 40.0):
	...
	for md in targets:
		if md.nat != null:
			var nfi: int = md.nat.pick_vertex(mouse, best_d, false)
			if nfi >= 0:
				var sp2: Vector2 = md.nat.screen_pos(nfi) + md.nat.fan_offset(nfi)
				var nd := sp2.distance_to(mouse)
				if nd < best_d:
					best_d = nd
					best = md.nat.get_color(nfi)
			continue
		for fi in md.vertex_count():
			var world := md.get_world_vertex(fi)
			if not is_in_front(camera, world):
