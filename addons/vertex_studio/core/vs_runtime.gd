...
const RESTORE_BTN := &"restore_base_instance"
## PROPERTY_HINT_TOOL_BUTTON by value (Godot 4.3 compat)
## TODO: drop once support to 4.3 is dropped and use `@export_tool_button`)
const TOOL_BUTTON_HINT := 39
const TOOL_BUTTON_MIN_VERSION := 0x040400
...
func _get_property_list() -> Array[Dictionary]:
	var tool_buttons: bool = Engine.get_version_info().get("hex", 0) >= TOOL_BUTTON_MIN_VERSION
	return [{
		"name": RESTORE_BTN,
		"type": TYPE_CALLABLE if tool_buttons else TYPE_BOOL,
		"hint": TOOL_BUTTON_HINT if tool_buttons else PROPERTY_HINT_NONE,
		"hint_string": "Restore base instance,Reload" if tool_buttons else "",
		"usage": PROPERTY_USAGE_EDITOR,
	}]


func _get(property: StringName) -> Variant:
	if property == RESTORE_BTN:
		if Engine.get_version_info().get("hex", 0) >= TOOL_BUTTON_MIN_VERSION:
			return restore_base_from_original_scene
		return false
	return null
...
func _set(property: StringName, value: Variant) -> bool:
	if property == RESTORE_BTN:
		if value is bool and value:
			restore_base_from_original_scene()
		return true
	return false
