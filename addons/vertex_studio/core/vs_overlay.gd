...
## Brush fill alpha for opacity 0%
const FILL_ALPHA_MIN := 0.05
## Brush fill alpha for opacity 100%
const FILL_ALPHA_MAX := 0.8
## Fixed visibility of the Paint Normals brush (it has no opacity)
const NORMALS_FILL_ALPHA := 0.15
...
var wants_redraw := false
var _md_settled: Dictionary = {}
...
func draw(overlay: Control, camera: Camera3D) -> void:
	...
	painter.sync_native_view(camera)
	painter.push_native_brush()
	wants_redraw = false
	_md_settled.clear()
	for md in painter.targets:
		if md.nat != null and not md.nat.overlay_view_settled():
			_md_settled[md.get_instance_id()] = false
			wants_redraw = true
...
## Wireframe edge cap for the native draw (2M-tri meshes would otherwise
## build a 12M-point multiline).
const WIREFRAME_MAX_EDGES := 300000
...
func _draw_wireframe(overlay: Control, camera: Camera3D) -> void:
	...
	for md in painter.targets:
		if not is_instance_valid(md.mesh_instance):
			continue
		if md.nat != null:
			if _md_settled.get(md.get_instance_id(), true):
				md.nat.overlay_wireframe(overlay.get_canvas_item(), col, WIREFRAME_MAX_EDGES)
			continue
...
func _draw_vertices(overlay: Control, camera: Camera3D) -> void:
	...
	for md in painter.targets:
		if not is_instance_valid(md.mesh_instance):
			continue
		if md.nat != null:
			if not _md_settled.get(md.get_instance_id(), true):
				continue
			var params := {
				"mouse": mouse,
				...
				"radius": r,
				"dot_size": base,
				"always_show": state.always_show_vertices,
				"brush_active": brush_active,
				"precision": precision,
				"normals": normals,
				"hide_unselected": hide_others,
				"sel_on": sel_on,
				"selecting": selecting,
				"marquee": sel_points,
				"marquee_type": sel_type,
				"max_verts": 100000,
				"ui_scale": EditorInterface.get_editor_scale(),
			}
			if _precision_md == md and not _precision_group.is_empty():
				params["skip_group"] = PackedInt32Array(_precision_group)
			if not _precision_lock.is_empty() and bool(_precision_lock["is_fan"]):
				params["skip_center"] = _precision_lock["center"]
				params["skip_radius"] = float(_precision_lock["radius"])
			md.nat.overlay_draw(overlay.get_canvas_item(), params)
			continue
...
func _draw_brush(overlay: Control) -> void:
	...
	if state.operation == VSState.Op.NORMALS:
		...
		alpha = NORMALS_FILL_ALPHA
	var fill := Color(base_col.r, base_col.g, base_col.b, alpha)
...
func _fill_alpha() -> float:
	return lerpf(FILL_ALPHA_MIN, FILL_ALPHA_MAX, clampf(state.opacity, 0.0, 1.0))
...
func _draw_precision(overlay: Control, camera: Camera3D) -> void:
	...
	if not _precision_lock.is_empty() and bool(_precision_lock["is_fan"]):
		...
		overlay.draw_circle(c, rad, Color(state.color.r, state.color.g, state.color.b, _fill_alpha()))
