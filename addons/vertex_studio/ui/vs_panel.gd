...
signal commit_mode_selected(mode: int)
...
var _commit_mode_opt: OptionButton
var _restore_hint: Control
	...
	for child in node.get_children():
		if child is Button:
			...
			if b.has_meta("vs_icon_gold"):
				...
				_tint_icon_button(b)
			if b.get_meta("vs_highlight", false):
				_highlight_restore(true)
...
func _build_targets(root: VBoxContainer) -> void:
	...
	_targets_section = body.get_parent().get_parent()
	_targets_section.visible = VSSettings.show_mesh_statistics()
	_targets_label = Label.new()
	_targets_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
...
func _build_material(root: VBoxContainer) -> void:
	...
	opt.item_selected.connect(func(idx): commit_mode_selected.emit(idx))
	_commit_mode_opt = opt
	row2.add_child(opt)
	_restore_btn = _icon_button("restore",
		"Restore Material: apply the \"On restore\" material now.",
		false, func(): action_requested.emit(&"commit_material"))
	row2.add_child(_restore_btn)
	body.add_child(row2)
	...
	var hint := Label.new()
	hint.text = "You have to click the Restore Material button and save the scene at least once."
	...
	_restore_hint = hint_row
	body.add_child(hint_row)
...
func _view_toggle(icon: Texture2D, tip: String, getter: Callable, setter: Callable) -> Button:
	...
	b.toggled.connect(func(v):
		...
		state.emit_changed())
	_syncers.append(func(): b.set_pressed_no_signal(getter.call()))
	return b
...
func _shared_verts_button(icon_name: String, tip: String, mode: int, group: ButtonGroup, feature := -1) -> Button:
	...
	if feature != -1 and VSPro.locked(feature):
		...
		b.pressed.connect(func(): show_pro_alert(feature))
		_gold_icon(b)
		return b
...
func _paint_op_button(icon_name: String, tip: String, op: int, feature := -1) -> Button:
	...
	var b := _icon_button(icon_name, tip, true, func(): toggle_brush_tool(op))
...
func _build_selection_fields(container: Control) -> void:
	...
	mlist.item_clicked.connect(func(idx: int, _at: Vector2, mb: int):
		if _material_list_updating:
			return
		if mb != MOUSE_BUTTON_LEFT:
			return
		if VSPro.locked(VSPro.Feature.SELECT_LINKED):
			show_pro_alert(VSPro.Feature.SELECT_LINKED)
			return
...
func _build_fill_normals(root: VBoxContainer) -> void:
	...
	if VSPro.locked(VSPro.Feature.PAINT_NORMALS):
		_fill_hard_btn.pressed.connect(func(): show_pro_alert(VSPro.Feature.PAINT_NORMALS))
	...
	if VSPro.locked(VSPro.Feature.PAINT_NORMALS):
		_fill_smooth_btn.pressed.connect(func(): show_pro_alert(VSPro.Feature.PAINT_NORMALS))
...
func _build_vgroups(root: VBoxContainer) -> void:
	...
	_vgroup_list.item_activated.connect(func(idx: int):
		...
		if VSPro.locked(VSPro.Feature.VERTEX_GROUPS):
			show_pro_alert(VSPro.Feature.VERTEX_GROUPS)
			return
		_vgroup_active = _vgroup_list.get_item_text(idx)
		_refresh_vgroup_buttons()
...
func _build_snapshot(root: VBoxContainer) -> void:
	...
	_snapshot_list.item_activated.connect(func(idx: int):
		...
		if VSPro.locked(VSPro.Feature.SNAPSHOTS):
			show_pro_alert(VSPro.Feature.SNAPSHOTS)
			return
		var p := str(_snapshot_list.get_item_metadata(idx))
		if p != "":
...
func set_commit_mode(mode: int) -> void:
	if _commit_mode_opt != null and _commit_mode_opt.selected != mode:
		_commit_mode_opt.selected = mode


func set_restore_hint(show: bool) -> void:
	if is_instance_valid(_restore_hint) and _restore_hint.visible != show:
		_restore_hint.visible = show
	_highlight_restore(show)


func _highlight_restore(on: bool) -> void:
	if not is_instance_valid(_restore_btn):
		return
	_restore_btn.set_meta("vs_highlight", on)
	if not on:
		_restore_btn.remove_theme_stylebox_override("normal")
		_restore_btn.remove_theme_stylebox_override("hover")
		_tint_icon_button(_restore_btn)
		return
	var accent := _ed_color("accent_color", "Editor", Color(0.4, 0.7, 1.0))
	for state_name in ["normal", "hover"]:
		var sb := _accent_outline(state_name, accent)
		if sb != null:
			_restore_btn.add_theme_stylebox_override(state_name, sb)
	for c in ["icon_normal_color", "icon_hover_color", "icon_focus_color"]:
		_restore_btn.add_theme_color_override(c, accent)


func _accent_outline(state_name: String, accent: Color) -> StyleBox:
	var t := EditorInterface.get_editor_theme()
	var base: StyleBox = t.get_stylebox(state_name, "Button") if t else null
	var sb: StyleBoxFlat = (base.duplicate() as StyleBoxFlat) if base is StyleBoxFlat else StyleBoxFlat.new()
...
func _channel_segmented() -> HBoxContainer:
	...
	for i in labels.size():
		...
		if i > 0 and pro_locked:
			b.toggle_mode = false
			b.tooltip_text = tip + " (PRO VERSION)"
			b.pressed.connect(func(): show_pro_alert(VSPro.Feature.SINGLE_CHANNEL))
			_gold_icon(b)
		else:
...
func _pro_wide_button(text: String, feature: int, cb: Callable) -> Button:
	if VSPro.locked(feature):
		var b := _wide_button(text, func(): show_pro_alert(feature))
		_gold_icon(b)
		return b
...
func _add_falloff_ui(container: Control) -> void:
	...
	if pro_locked:
		ce.clicked.connect(func(): show_pro_alert(VSPro.Feature.FALLOFF))
	else:
...
func adjust_opacity(delta: float) -> void:
	state.opacity = clampf(state.opacity + delta, 0.0, 1.0)
	_sync_ui()
	state.emit_changed()


func toggle_brush_tool(op := -1) -> void:
	_cancel_eyedrop()
...
func set_target_info(mesh_count: int, vert_count: int, selected_count: int = 0, tri_count: int = -1) -> void:
	if _targets_label == null or (is_instance_valid(_targets_section) and not _targets_section.visible):
		return
	if mesh_count == 0:
		_targets_label.text = "No paintable mesh selected"
	else:
		var txt := "%d mesh%s, %s verts" % [
			mesh_count, "" if mesh_count == 1 else "es", _grouped(vert_count)]
		if tri_count >= 0:
			txt += ", %s tris" % _grouped(tri_count)
		if selected_count > 0:
			txt += "  (%s selected)" % _grouped(selected_count)
		_targets_label.text = txt


func set_show_statistics(on: bool) -> void:
	if not is_instance_valid(_targets_section) or _targets_section.visible == on:
		return
	_targets_section.visible = on
	if on:
		action_requested.emit(&"refresh_target_info")
...
func _pro_icon_button(icon_name: String, tip: String, toggle: bool, feature: int, cb: Callable) -> Button:
	if VSPro.locked(feature):
		var b := _icon_button(icon_name, tip + " (PRO VERSION)", false, func(): show_pro_alert(feature))
		_gold_icon(b)
		return b
...
func show_pro_alert(feature := -1) -> void:
	if _pro_dialog == null:
	...
	_pro_dialog.dialog_text = VSPro.alert_text(feature)
